
import { Property } from "@/types/property";

export const property7: Property = {
  id: 7,
  title: "Studio , 1 and 2 Bedrooms",
  address: "westlands",
  price: 4250000,
  discountedPrice: 3950000,
  bedrooms: 6,
  bathrooms: 5.5,
  area: 6200,
  description: "📍 Prime Location: Sports Road, Westlands This iconic 25-floor development blends residential, hotel apartments, and commercial spaces, offering a high-end urban lifestyle in one of Nairobi’s most sought-after locations. 🏢 Building Layout: 5th – 14th Floor: Residential Apartments (Studio, 1 & 2 Bedrooms) 15th – 24th Floor: Fully Serviced Hotel Apartments 6 Levels of Parking 💰 Unit Prices & Sizes 🏠 Studio (41 sqm) – $60,270 🏠 1-Bedroom (70 sqm) – $102,900 🏠 2-Bedroom (90 sqm) – $132,300 📆 Flexible 30-Month Payment Plan ✅ 15% on reservation ✅ 15% on signing the Sale Agreement ✅ 50% spread over construction ✅ 20% on handover",
  features: [
    "Modern & Spacious Apartments – Thoughtfully designed layouts for comfortable living.",
"Premium Finishes – High-end materials and elegant interiors.",
"Smart Home Features – Video intercoms and security access control.",
"Floor-to-Ceiling Windows – Breathtaking city skyline views.",
"Fully Equipped Kitchens – High-quality appliances and contemporary fittings.",
"Luxurious Bathrooms – Stylish fixtures and top-notch craftsmanship.",
"Balconies with Scenic Views – Enjoy the beauty of Westlands."
  ],
  image: "/43.webp",
  images: [
    "/41.webp",
    "/40.webp",
    "/42.jpeg",
    "/43.webp"
  ],
  type: "House",
  status: "For Sale",
  communityAmenities: [
    "24/7 Double-Volume Reception & Valet Parking – Exclusive concierge services.",
"High-Level Security – CCTV, 24-hour security, and controlled access.",
"5-Star Restaurant & 24-Hour Convenience Store – Dining & shopping at your doorstep.",
"Office Spaces & Business Hub – Ideal for remote work and meetings.",
"Sauna, Steam Room & Massage Rooms – Relaxation and wellness facilities.",
"Yoga Garden, Zumba & Aerobics Studio – Stay active and healthy.",
"Children's Daycare & Pet Facility – Family-friendly living.",
"Fully Equipped Gym & Rooftop Infinity Pool with Jacuzzi – Unparalleled fitness and leisure experience.",
"Luxury Unisex Salon – Professional grooming services.",
"Panoramic Rooftop Bar & Cigar Lounge – Elegant social spaces.",
"Fully Backed Generator – Uninterrupted power supply.",
"Coffee Place & Lounge Area – Perfect for work or relaxation."
  ],
  
};
